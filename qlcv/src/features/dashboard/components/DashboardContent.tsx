...
export function DashboardContent({ summary, range }: { summary: DashboardSummary; range: DashboardRange }) {
  ...
  return (
    <>
      <DashboardStatusStrip summary={summary} paymentBlocks={paymentBlocks} />
      <DashboardKpiGrid
        summary={summary}
        range={range}
      ...
      <div className="od-dashboard__grid">
        <div className="od-dashboard__stack">
          <div className="od-dashboard__split-grid">
            ...
            <WorkflowDistribution summary={summary} onOpen={(column) => router.push(`/projects?macro_column=${column}`)} />
          </div>
          ...
          <div className="od-dashboard__split-grid">
            ...
            <Panel title="OKR summary" subtitle="Cycle hiện tại"><OkrSummary /></Panel>
          </div>
        ...
        <DashboardActions summary={summary} />
      </div>
    </>
  );
}

function DashboardStatusStrip({ summary, paymentBlocks }: { summary: DashboardSummary; paymentBlocks: number }) {
  return (
...
function DashboardKpiGrid({ summary, range, overdueProjects, highWorkloadCount }: { summary: DashboardSummary; range: DashboardRange; overdueProjects: number; highWorkloadCount: number }) {
  return (
...
function WorkflowDistribution({ summary, onOpen }: { summary: DashboardSummary; onOpen: (column: string) => void }) {
  return (
      ...
      {summary.project_workflow.length === 0 ? (
        ...
        <div className="od-dashboard__workflow-list">
          {summary.project_workflow.map((item) => (
            <button key={item.macro_column} className="od-dashboard__workflow-item" type="button" onClick={() => onOpen(item.macro_column)}>
              <span>
...
function DashboardActions({ summary }: { summary: DashboardSummary }) {
  return (
