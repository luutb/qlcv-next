...
import {
  Typography, Box, Paper, Grid, CircularProgress, Button,
} from '@mui/material';
import { Assignment, PendingActions, CheckCircle, Paid } from '@mui/icons-material';
...
export default function ManagerDashboard() {
  ...
  const cards = [
    { label: 'Tổng hồ sơ', value: stats?.tasks.total ?? 0, icon: <Assignment sx={{ fontSize: 40 }} />, color: '#1976d2' },
    ...
    { label: 'Từ chối', value: stats?.tasks.rejected ?? 0, icon: <Paid sx={{ fontSize: 40 }} />, color: '#d32f2f' },
  ];
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          <Typography variant="h4" fontWeight={700}>Tổng quan</Typography>
          <Typography color="text.secondary">Xin chào, {user?.full_name}.</Typography>
        </Box>
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => router.push('/manager/tasks/create')}>
          Tạo hồ sơ
        </Button>
      </Box>
