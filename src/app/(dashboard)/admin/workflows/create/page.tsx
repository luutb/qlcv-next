...
const FIXED_STEPS = ['Tiếp nhận', 'Xác nhận thanh toán', 'Hoàn tất', 'Thu tiền', 'Hoàn thành'];
...
export default function CreateWorkflowPage() {
  ...
  } = useForm<WorkflowForm>({ defaultValues: { name: '', description: '' } });
        ...
        <Paper key={index} variant="outlined" sx={{ p: 2 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            <TextField
              ...
              onChange={(e) => updateStep(setSteps, index, 'step_name', e.target.value)}
              size="small" sx={{ flex: 1, minWidth: 200 }}
              error={!!stepErrors[`${prefix}_${index}`]}
              helperText={stepErrors[`${prefix}_${index}`]}
            />
            <TextField
              select label="Vai trò"
              value={step.required_role}
              onChange={(e) => updateStep(setSteps, index, 'required_role', e.target.value)}
              size="small" sx={{ minWidth: 150 }}
            >
              {ROLE_OPTIONS.map((r) => <MenuItem key={r.value} value={r.value}>{r.label}</MenuItem>)}
            </TextField>
            <TextField
              select label="Yêu cầu file"
              value={step.require_file ? 'true' : 'false'}
              onChange={(e) => updateStep(setSteps, index, 'require_file', e.target.value === 'true')}
  ...
  return (
    <Box component="form" onSubmit={handleSubmit(onSubmit)}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
        ...
        <Typography variant="h5" fontWeight="bold">Tạo quy trình mới</Typography>
      </Box>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '2fr 1fr' }, gap: 3 }}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          <Paper sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <TextField
                label="Tên quy trình" required fullWidth
                {...register('name', { required: 'Tên quy trình không được để trống' })}
                error={!!errors.name} helperText={errors.name?.message}
              />
              <TextField label="Mô tả" {...register('description')} multiline rows={3} fullWidth />
            </Box>
          </Paper>
            ...
            {previewSteps.map((stepName, index) => {
              ...
              return (
                <Box key={index} sx={{
                  ...
                  <Chip label={index + 1} size="small" color={isFixed ? 'default' : 'primary'}
                    variant="outlined" sx={{ minWidth: 32 }} />
                  <Typography variant="body2" fontWeight={isFixed ? 400 : 600}>{stepName}</Typography>
                  {isFixed && <Chip label="Cố định" size="small" variant="outlined" sx={{ ml: 'auto', fontSize: 10 }} />}
                </Box>
              );
            })}
          </Box>
